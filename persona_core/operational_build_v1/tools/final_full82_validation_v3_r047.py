"""Third clean full82 validation after final_full82_02 stopped on remote UNKNOWN.

The parent UNKNOWN is preserved and quarantined; it is never rewritten or
resent. This revision uses a fresh logical batch/runtime/call identity while
keeping the same frozen 82 inputs, 328 criteria, provider roles, thinking
profile and quality thresholds. Creation/preparation are zero-call operations;
execution requires an exact >=20 CNY user authorization for this batch only.
"""
...
import final_full82_validation_v2_r047 as previous

ROOT, PLAN = common.ROOT, common.PLAN
REV = PLAN / 'evidence/R047-03/final_full82_03'
LIVE = REV / 'live'
BATCH = 'APCORE-R047-FINAL-FULL82-03'
GUARD_CNY = 125
AGGREGATE_GUARD_CNY = 868.5
TARGETED = PLAN / 'evidence/R047-03/TARGETED_DIAGNOSTIC_CLOSEOUT.json'
PARENT = PLAN / 'evidence/R047-03/final_full82_02'
...
def fixed_scope():
    ...
    scope.update(
        batch_id=BATCH,
        purpose=('Third clean final full82/328 known-development validation after '
                 'final_full82_02 terminal remote UNKNOWN; no parent call or slot is retried.'),
    ...
    ensure(len(scope['slots']) == 82, 'Final03 frozen denominator changed')
    ensure(sum(s['model'] == 'deepseek-v4-flash' for s in scope['slots']) == 76, 'Final03 Flash denominator changed')
    ensure(sum(s['model'] == 'deepseek-v4-pro' for s in scope['slots']) == 6, 'Final03 Pro denominator changed')
    ensure(scope['reserved_upper_micro_cny'] <= GUARD_CNY * 1_000_000, 'Final03 reserve exceeds guard')
    return scope, cases, fixture, frozen


def parent_failure() -> dict:
    proof = common.read_json(PARENT_FAILURE)
    ensure(proof['status'] == 'TERMINAL_STOPPED_46_CAPTURED_1_REMOTE_UNKNOWN_35_NOT_SUBMITTED',
           'Parent final02 terminal class changed')
    ensure(proof['batch_id'] == 'APCORE-R047-FINAL-FULL82-02', 'Wrong parent final02 batch')
    journal = ROOT / proof['journal']
    audit = ROOT / proof['capture_audit']
    ensure(common.file_sha(journal) == proof['journal_sha256'], 'Parent final02 journal changed')
    ensure(common.file_sha(audit) == proof['capture_audit_sha256'], 'Parent final02 audit changed')
    db = sqlite3.connect(journal.as_uri() + '?mode=ro', uri=True)
    db.row_factory = sqlite3.Row
    try:
        ensure(db.execute('PRAGMA integrity_check').fetchone()[0] == 'ok', 'Parent final02 journal integrity failure')
        counts = dict(db.execute('SELECT status,count(*) FROM provider_calls GROUP BY status').fetchall())
        ensure(counts == {'RESPONSE_CAPTURED': 46, 'SUBMITTED_STATUS_UNKNOWN': 1}, 'Parent final02 counts changed')
        batch = db.execute('SELECT stopped FROM call_batches WHERE batch_id=?', (proof['batch_id'],)).fetchone()
        ensure(batch is not None and batch[0] == 1, 'Parent final02 must stay stopped')
        driver = db.execute("SELECT value FROM metadata WHERE key='r047_active_driver'").fetchone()
        ensure(driver is None or driver[0] == '', 'Parent final02 still has active driver')
        row = db.execute(
            "SELECT p.call_id,p.status,p.http_status,p.error_category,p.request_sha256,p.raw_sha256,p.usage_json,"
            "p.reserve_micro_cny,t.status AS turn_status,t.assistant_text,p.turn_id "
            "FROM provider_calls p JOIN turns t USING(turn_id) WHERE p.batch_id=? AND p.slot_id='N08_T3'",
            (proof['batch_id'],)).fetchone()
        ensure(row is not None and row['call_id'] == proof['failed_call_id'], 'Parent final02 failed call identity changed')
        ensure(row['status'] == 'SUBMITTED_STATUS_UNKNOWN' and row['turn_status'] == 'SUBMITTED_STATUS_UNKNOWN',
               'Parent final02 failed slot status changed')
        ensure(row['http_status'] is None and row['raw_sha256'] is None and row['usage_json'] is None,
               'Parent final02 UNKNOWN unexpectedly acquired a provider result')
        ensure(row['error_category'] == 'PROVIDER_WORKER_REMOTE_OUTCOME_UNKNOWN/URLError',
               'Parent final02 UNKNOWN category changed')
        ensure(row['assistant_text'] is None, 'Parent final02 UNKNOWN unexpectedly has assistant text')
        ensure(db.execute('SELECT count(*) FROM display_journal WHERE turn_id=?', (row['turn_id'],)).fetchone()[0] == 0,
               'Parent final02 UNKNOWN unexpectedly displayed')
        ensure(db.execute('SELECT count(*) FROM chat_traces WHERE turn_id=?', (row['turn_id'],)).fetchone()[0] == 0,
               'Parent final02 UNKNOWN unexpectedly committed chat/event state')
    finally:
        db.close()
    audit_value = common.read_json(audit)
    ensure(audit_value['status_counts'] == {'NOT_SUBMITTED': 35, 'RESPONSE_CAPTURED': 46, 'SUBMITTED_STATUS_UNKNOWN': 1},
           'Parent final02 audit denominator changed')
    ensure(audit_value['binding_findings'] == [] and audit_value['complete_captures_and_delivery'] is False,
           'Parent final02 audit was upgraded or corrupted')
    quarantined = unknowns.validated_quarantined_unknowns()
    ensure(proof['failed_call_id'] in quarantined, 'Parent final02 UNKNOWN is not quarantined')
    q = quarantined[proof['failed_call_id']]
    ensure(q['classification'] == 'QUARANTINED_REMOTE_UNKNOWN' and q['resend_allowed'] is False,
           'Parent final02 quarantine boundary changed')
    return {
        ...
        'error_category': 'PROVIDER_WORKER_REMOTE_OUTCOME_UNKNOWN/URLError',
        'captured': 46, 'rejected': 0, 'not_submitted': 35, 'unknown': 1,
        'quarantine_manifest': q['manifest'],
        'quarantine_manifest_sha256': q['manifest_sha256'],
    }


def targeted_closeout() -> dict:
    return previous.targeted_closeout()
...
def current_regression() -> dict:
    ...
    raise common.StoreGuard('No current-source regression binds final03 full82')


def readiness(require_regression: bool = True) -> dict:
    unknown_status = unknowns.ensure_new_independent_batch_allowed()
    ensure(not unknown_status['active_unresolved_remote_unknowns'], 'Active remote UNKNOWN blocks final03')
    result = {'unknown_policy': unknown_status,
              'targeted_closeout': targeted_closeout(),
              'parent_final02_failure': parent_failure()}
...
def execution_authorization(scope: dict) -> dict:
    ...
    ensure(path.is_file(), 'Explicit >=20 CNY final03 authorization required')
    auth = common.read_json(path)
    ensure(auth.get('approved') is True and auth.get('batch_id') == BATCH, 'Final03 authorization does not bind this batch')
           ...
           'Final03 authorization parameters changed')
    return auth
...
def load(prepare: bool = False):
    ...
    manifest = common.read_json(REV / 'MANIFEST.json')
           ...
           and common.file_sha(REV / 'EXECUTION_SCOPE.json') == manifest['scope_sha256'], 'Final03 scope changed')
    ensure(manifest['fixed_calls'] == 82 and manifest['guard_cny'] == GUARD_CNY, 'Final03 manifest denominator/guard changed')
    ensure(manifest['parent_final02_failure'] == parent_failure(), 'Final03 parent failure binding changed')
...
def create() -> int:
    ...
    ensure(not REV.exists(), 'Final03 revision already exists')
    ...
    dump(REV / 'MANIFEST.json', {
        ...
        'fixed_calls': 82, 'main_flash_calls': 76, 'switch_pro_calls': 6,
        'guard_cny': GUARD_CNY, 'aggregate_guard_cny': AGGREGATE_GUARD_CNY,
        'reserved_upper_micro_cny': scope['reserved_upper_micro_cny'],
        'readiness': ready, 'parent_final02_failure': ready['parent_final02_failure'],
        'known_development_regression': True, 'new_heldout_claim': False,
        'automatic_paid_retries': 0, 'automatic_next_revision': False,
        'full82_acceptance_claim': False, 'execution_requires_explicit_user_confirmation': True,
...
def dispatch(action: str, rest: list[str]) -> int:
    ...
    if action in {'prepare', 'execute'}:
        now = readiness()
        ensure(now['targeted_closeout'] == pinned['targeted_closeout'], 'Targeted closeout changed after final03 freeze')
        ensure(now['parent_final02_failure'] == pinned['parent_final02_failure'], 'Parent final02 failure changed')
        ensure(not now['unknown_policy']['active_unresolved_remote_unknowns'], 'New active remote UNKNOWN appeared')
    if action == 'prepare':
        ensure(not rest and not LIVE.exists(), 'Existing final03 runtime cannot be prepared twice')
    ...
    if action == 'execute':
        ...
        ensure(prepared['fixture_preparation_sha256'] == common.file_sha(LIVE / 'FIXTURE_PREPARATION.json'), 'Final03 preparation changed')
        official = common.read_json(REV / 'OFFICIAL_PREFLIGHT.json')
               ...
               and official['execution_recheck_complete'] is True, 'Final03 provider recheck stale/incomplete')
    ...
    previous_values = {key: getattr(module, key) for key in replacement}
    ...
    try:
        ...
        if action == 'prepare':
            ...
            ensure(preflight['passed'] and preflight['target_calls'] == 0, 'Final03 preparation failed')
        ...
        for key, value in previous_values.items(): setattr(module, key, value)
        sys.argv = old_args
