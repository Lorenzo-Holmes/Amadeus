"""Third named R047 semantic-repair regression revision.

repair_01 and repair_02 are immutable terminal batches. Both observed a real
provider response with HTTP 200 / finish_reason=stop / non-empty reasoning but
an empty final content field. The current candidate adds an expression-layer
instruction requiring a displayable final answer after internal reasoning.

This module starts a fresh full-82 logical batch. It never unlocks or resends a
slot inside either prior repair batch and retains zero automatic paid retries.
"""
...
REV = PLAN / 'evidence/R047-03/repair_03'
LIVE = REV / 'live'
PREVIOUS = PLAN / 'evidence/R047-03/repair_02'
BATCH = 'APCORE-R047-SEMANTIC-REPAIR-03'
PREVIOUS_BATCH = 'APCORE-R047-SEMANTIC-REPAIR-02'
KNOWN = 'KNOWN_DEVELOPMENT_REGRESSION'
TOTAL_GUARD_CNY = 45
ALL_OPERATIONS_GUARD_CNY = 181.5
...
def revision_scope(parent: dict) -> dict:
    ...
    scope.update(
        batch_id=BATCH,
        purpose=(
            'Fresh full82 known-case semantic repair regression after two preserved '
            'provider reasoning-only EMPTY_RESPONSE terminals and a minimal final-output '
            'expression repair. No prior repair slot is unlocked or retried in place.'
        ),
...
def taxonomy() -> dict:
    return {
        ...
        'provider_reliability_repair': 'NONEMPTY_DISPLAYABLE_FINAL_ANSWER_AFTER_REASONING',
        'parent_taxonomy_note': (
            'All repair_03 outputs are known development regression. Original NEW_AT_FREEZE '
            'taxonomy belongs only to the immutable parent protocol.'
        ),
    }
...
def verify_previous_terminal() -> dict:
    ...
    ensure(db_path.is_file(), 'repair_02 journal missing')
    db = sqlite3.connect(db_path.as_uri() + '?mode=ro', uri=True)
    db.row_factory = sqlite3.Row
    try:
        ensure(db.execute('PRAGMA integrity_check').fetchone()[0] == 'ok', 'repair_02 SQLite integrity failure')
        counts = {row['status']: row['n'] for row in db.execute(
            'SELECT status,count(*) AS n FROM provider_calls GROUP BY status')}
        ensure(counts == {'RESPONSE_CAPTURED': 27, 'RESPONSE_REJECTED': 1},
               'repair_02 terminal counts differ from preserved failure')
        batch = db.execute('SELECT stopped FROM call_batches WHERE batch_id=?', (PREVIOUS_BATCH,)).fetchone()
        ensure(batch is not None and batch['stopped'] == 1, 'repair_02 is not terminal-stopped')
        active = db.execute("SELECT value FROM metadata WHERE key='r047_active_driver'").fetchone()
        ensure(active is not None and active['value'] == '', 'repair_02 driver lease is not reconciled')
        ensure(db.execute("SELECT count(*) FROM provider_calls WHERE status='SUBMITTED_STATUS_UNKNOWN'").fetchone()[0] == 0,
               'repair_02 has unknown provider submissions')
        failed = db.execute(
        ...
        ensure(failed is not None and failed['slot_id'] == 'N05_T4', 'repair_02 rejected slot changed')
        ensure(failed['http_status'] == 200 and failed['finish_reason'] == 'stop' and failed['error_category'] == 'EMPTY_RESPONSE',
               'repair_02 rejection signature changed')
        raw = json.loads(failed['raw_response'])
        usage = json.loads(failed['usage_json'])
        message = raw['choices'][0]['message']
        reasoning = (usage.get('completion_tokens_details') or {}).get('reasoning_tokens')
        ensure(message.get('content') == '', 'repair_02 preserved response is no longer empty')
        ensure(reasoning == usage['completion_tokens'] == 38,
               'repair_02 preserved empty-response token signature changed')
        ensure(isinstance(message.get('reasoning_content'), str) and message['reasoning_content'].strip(),
               'repair_02 preserved reasoning content is missing')
        return {
...
def load_revision():
    ...
    scope = original.read_json(REV / 'EXECUTION_SCOPE.json')
           ...
           'repair_03 scope bytes differ from manifest')
    ensure(manifest['parent_scope_file_sha256'] == frozen['scope_sha256'], 'repair_03 parent protocol changed')
    ensure(scope == revision_scope(parent), 'repair_03 scope differs beyond declared repair revision')
    ensure(manifest['previous_repair_batch_id'] == PREVIOUS_BATCH, 'repair_03 lineage changed')
    ensure(manifest['known_development_regression'] is True and manifest['new_heldout_claim'] is False,
           'repair_03 cannot claim heldout evaluation')
    ensure(manifest['total_guard_cny'] == TOTAL_GUARD_CNY and manifest['all_operations_guards_cny'] == ALL_OPERATIONS_GUARD_CNY,
           'repair_03 guard accounting changed')
...
def create_revision() -> int:
    ...
    dump(REV / 'REVISION.json', {
        ...
        'older_terminal_evidence': 'evidence/R047-03/repair_01/live/runtime.sqlite3',
        'repair_evidence': 'evidence/R047-03/provider_output_fix_20260911/README.md',
        'continuation_policy': (
            'repair_01 and repair_02 remain immutable stopped batches. repair_03 uses a '
            'fresh full82 denominator after a product expression change, so no successful '
            'subset from an earlier attempt is cherry-picked.'
        ),
    })
    dump(REV / 'OFFICIAL_PREFLIGHT.json', {
        ...
        'model_note': (
            ...
            'with deepseek-v4-flash version/alias wording. Frozen model names are retained; '
            'any provider rejection remains terminal and is never silently substituted.'
        ),
...
def dispatch(action: str, rest: list[str]) -> int:
    load_revision()
    ensure(action in {'prepare', 'execute', 'audit', 'review'}, 'Unsupported repair_03 action')
    if action == 'prepare':
        ensure(not rest, 'Preparation does not accept extra arguments')
        ensure(not LIVE.exists(), 'Existing repair_03 runtime must be reconciled; preparation is never repeated')
    ...
    if action == 'execute':
        completed = original.read_json(REV / 'PREPARE_COMPLETE.json')
               ...
               'repair_03 preparation binding changed')
    ...
    try:
        ...
        if action == 'prepare':
            ...
            ensure(preflight['passed'] and preflight['target_calls'] == 0, 'repair_03 preparation did not complete')
