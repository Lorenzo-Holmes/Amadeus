"""Offline safety tests for repair_03. No target provider calls."""
...
import repair_revision_r047_v3 as wrapper
...
def offline_chat(store, handle, scope):
    ...
    def transport(payload, key):
        ...
        return 200, canonical({
            'model': body['model'],
            'choices': [{'finish_reason': 'stop', 'message': {'content': 'repair_03 离线流程测试答复。'}}],
...
class Repair03WrapperTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.repair02_before = hashes(wrapper.PREVIOUS)
    ...
    def test_previous_terminal_signature(self):
        ...
        self.assertEqual(state['status_counts'], {'RESPONSE_CAPTURED': 27, 'RESPONSE_REJECTED': 1})
        self.assertEqual(state['terminal_failure']['slot_id'], 'N05_T4')
        self.assertEqual(state['terminal_failure']['reasoning_tokens'], 38)
    ...
    def test_prior_and_original_unchanged(self):
        self.assertEqual(hashes(wrapper.PREVIOUS), self.repair02_before)
...
def main():
    ...
    with (OUT / 'TESTS.log').open('x', encoding='utf-8') as stream:
        result = unittest.TextTestRunner(stream=stream, verbosity=2).run(
            unittest.defaultTestLoader.loadTestsFromTestCase(Repair03WrapperTests))
    prior_ok = hashes(wrapper.PREVIOUS) == Repair03WrapperTests.repair02_before
    original_ok = hashes(wrapper.original.LIVE) == Repair03WrapperTests.original_before
    report = {
