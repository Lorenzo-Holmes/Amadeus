...
8. [KNOWN｜置信度：高] [ADR-001](outputs/ADR-001-Amadeus身份与成长模型.md) → [ADR-002](outputs/ADR-002-Amadeus记忆生命周期.md) → [ADR-003](outputs/ADR-003-Amadeus主动性与事件循环.md) → [ADR-004](outputs/ADR-004-Amadeus工具权限与执行治理.md) → [ADR-005](outputs/ADR-005-Amadeus关系安全与退出协议.md) — 支持性裁决。
9. [KNOWN｜置信度：高] [身份与记忆评测基线](outputs/Amadeus身份与记忆评测基线-v0.1.md) 是 Draft / 待实现；[主动性、权限及关系安全评测增量](outputs/Amadeus主动性权限与关系安全评测增量-v0.1.md) 是 Frozen-candidate；两者都尚待转为可执行 fixture。
10. [KNOWN｜置信度：高] [项目开发知识库导航](knowledge/data_structure.md) — 00/10/20/30/40/90/99 分层及更新合同。
...
- [ ] [KNOWN] 运行项目知识库 check；当前应报告 30 个索引文档与 0 个 raw 路径。
