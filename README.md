...
[KNOWN｜置信度：高] 这个仓库尚不是已经完成的 Amadeus v0.1，也不是可直接发布的聊天产品。当前可核验完成项是 Stage 0A 来源工具链；行为 fixture、确定性 Core 与模型接入仍在后续阶段。
...
| [KNOWN] 待显式分配 oracle | [COMPUTED] 95 |
| [KNOWN] 待人工裁决 atomicity | [COMPUTED] 214 |
| [KNOWN] 当前 Core / Stage 0 readiness 声明 | [KNOWN] 只有 <code>source_toolchain_ready=true</code> |
...
1. [KNOWN｜置信度：高] **Stage 0B**：固定 Stage 0A 四份 generated JSON 身份；人工完成 95 个 Core oracle 分配、214 个 atomicity 裁决和 source-clause manifest。
2. [INFERRED｜置信度：高] **Stage 0C**：仅在 0B 门禁通过后实现 fixture DSL、clause→case 绑定与 S 动作沙箱。
...
[KNOWN｜置信度：高] 不从 Stage 0A 跳到 0C，不在 fixture 裁决完成前把模型输出当作 Core 基线。

## Backlog

- [KNOWN｜置信度：高] Stage 0B 的 95 个 oracle、214 个 atomicity 裁决与 source-clause manifest。
